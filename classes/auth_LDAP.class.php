...
//echo 'pass='.$pass."\n";
		...
		# START: AUTHENTICATION against LDAP server
			...
			if (!$user) {
				$ldap_msg = "No username specified.";
			...
			if (!$pass) {
				$ldap_msg = "No password specified.";
				error_reporting($errorLevel);
				return FALSE;
			}
#return "preConnected to $ldap_server.<br/>";

			// Connect to the LDAP server.
			# DKC: LIVE  SERVER: $ldap_server, 636
			# DKC: LOCAL SERVER: $ldap_server
			if (($connect = @ldap_connect($ldap_server)) == FALSE) {
				$ldap_msg = "Could not connect to the LDAP server
	            ($ldap_server).  " . ldap_error($connect);
			...
			if (($res_id = ldap_search($connect,
				...
				$ldap_msg = "Could not find the user in the LDAP tree.";
				error_reporting($errorLevel);
				return FALSE;
			}
			...
			if (($entry_id = ldap_first_entry($connect, $res_id)) == FALSE) {
				$ldap_msg = "User record could not be fetched.";
				error_reporting($errorLevel);
				return FALSE;
			}
			...
			if (($user_dn = ldap_get_dn($connect, $entry_id)) == FALSE) {
				$ldap_msg = "The user-dn could not be fetched.";
				error_reporting($errorLevel);
				return FALSE;
			}
			...
			if (($link_id = ldap_bind($connect, $user_dn, $pass)) == FALSE) {
				# failed log in
				$ldap_msg = "The username and password don't match."; //: $user_dn";
				error_reporting($errorLevel);
				return FALSE;
			} else {
				# successful log in
				# if ($chat) echo "Bound successfully with user-dn and password.<br/>";
				$ldap_msg = "Woohoo! The username and password are a match."; //: $user_dn";
				@ldap_close($connect);
				error_reporting($errorLevel);
				return TRUE;
			}
