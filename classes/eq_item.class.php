<?php
	...
	class EqItem extends Db_Linked {
		public static $fields = array('eq_item_id', 'eq_subgroup_id', 'name', 'descr', 'reference_link', 'image_file_name', 'flag_image_to_be_uploaded', 'ordering', 'flag_delete');
		...
		public static function cmp($a, $b) {
			...
			$aLoad = TRUE;
			$bLoad = TRUE;
			if (!isset($a->eq_subgroup)) {
				$aLoad = $a->loadEqSubgroup();
			}
			if (!isset($b->eq_subgroup)) {
				$bLoad = $b->loadEqSubgroup();
			}
			if ($aLoad && $bLoad) {
		...
		public function loadEqGroup() {
			if (!isset($this->eq_subgroup)) {
				if (!$this->loadEqSubgroup()) {
					return FALSE;
				}
			}
			if (!isset($this->eq_subgroup->eq_group)) {
				if (!$this->eq_subgroup->loadEqGroup()) {
					return FALSE;
				}
			}
